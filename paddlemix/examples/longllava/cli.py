...
import numpy as np
import uuid
from paddlemix.models.llava import *
from paddlemix.models.llava.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
from paddlemix.models.llava.language_model.llava_jamba import LlavaJambaForCausalLM
...
from paddlemix.models.llava.mm_utils import tokenizer_image_token, process_images
from paddlenlp.transformers import JambaTokenizer


class Chatbot:

    def __init__(self, config) ->None:
        self.gen_kwargs = {'do_sample': False, 'max_new_tokens': 768,
            'min_new_tokens': 1, 'temperature': 0.0}
    ...
    def tokenizer_image_token(self, prompt, image_token_index=
        IMAGE_TOKEN_INDEX, return_tensors=None):
        prompt_chunks = [self.tokenizer(chunk, add_special_tokens=False).
    ...
    def chat_with_jamba(self, text: str, media=None, isVideo=False, t=1.0, frameNum=128, patchside_length=336, patchStrategy='norm'):
            ...
            try:
                ...
                while True:
                    ...
                    if count % frame_interval == 0:
                        pil_img = Image.fromarray(cv2.cvtColor(frame, cv2.
                            COLOR_BGR2RGB))
                        frameList.append(pil_img)
                    count += 1
                cap.release()
                if len(frameList) > frameNum:
                    indices = np.linspace(0, len(frameList) - 1, frameNum,
                        dtype=int)
        ...
        def processForBestFitPatch(text, images, output_dir=
            './LongLLaVA/data/TestBestFit', patchside_length=336):
            ...
            for idx, image_path in enumerate(images):
                if isinstance(image_path, str) and os.path.isfile(image_path):
                    ...
                    image = cv2.cvtColor(np.array(image_path), cv2.
                        COLOR_RGB2BGR)
                    random_filename = str(uuid.uuid4()) + '.jpg'
                    random_path = os.path.join(output_dir, 'ori', 'images',
                        random_filename)
                ...
                new_height = (height + side_length - 1
                    ) // side_length * side_length
                new_width = (width + side_length - 1
                    ) // side_length * side_length
                pad_height = (new_height - height) // 2
                pad_width = (new_width - width) // 2
                padded_image = cv2.copyMakeBorder(image, pad_height,
                    pad_height, pad_width, pad_width, cv2.BORDER_CONSTANT,
                    value=[255, 255, 255])
                split_images = [image_path]
                path_parts = image_path.split('/')
                base_name = os.path.splitext(path_parts[-1])[0]
                if len(path_parts) >= 3:
                    subdir = os.path.join(output_dir, path_parts[-3],
                        path_parts[-2])
                else:
                ...
                for i in range(0, new_height, side_length):
                    for j in range(0, new_width, side_length):
                        split_img = padded_image[i:i + side_length, j:j +
                            side_length]
                        split_path = os.path.join(subdir,
                            f'{base_name}_{i // side_length}_{j // side_length}.jpg'
                            )
                ...
                replace_str = '<image>\n' + '\n'.join([('<img>' +
                    '</img><img>'.join(['<image>' for _ in range(col_count)
                    ]) + '</img>') for _ in range(row_count)])
        ...
        if isinstance(media, str) or isinstance(media, Image.Image
            ) or isinstance(media, cv2.VideoCapture) or media is None:
            media = [media]
        images = []
        VideoFLAG = False
        ...
        if len(images):
            if patchStrategy == 'bestFit':
                text, images = processForBestFitPatch(text, images,
    ...
    def chat(self, text: str, images: list[str]=None, isVideo=False, t=1.0,
