...
from paddlemix.models.llava.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, \
...
class Chatbot():
    ...
    def init_components(self):
        ...
        model = LlavaJambaForCausalLM.from_pretrained(d, dtype=paddle.bfloat16).eval()
    ...
    def tokenizer_image_token(self, prompt, image_token_index=IMAGE_TOKEN_INDEX,
                              return_tensors=None):  # copied from llava
    ...
    def chat_with_jamba(self, text: str, media=None, isVideo=False, t=1.0, frameNum=128, patchside_length=336,
        ...
        def processForBestFitPatch(text, images, output_dir='./LongLLaVA/data/TestBestFit', patchside_length=336):
            ...
            for idx, image_path in enumerate(images):
                ...
                padded_image = cv2.copyMakeBorder(image, pad_height, pad_height, pad_width, pad_width,
                ...
                replace_str = '<image>\n' + '\n'.join(
                    ['<img>' + '</img><img>'.join(['<image>' for _ in range(col_count)]) + '</img>' for _ in
                     range(row_count)])
        ...
        if isinstance(media, str) or isinstance(media, Image.Image) or isinstance(media,
                                                                                  cv2.VideoCapture) or media is None:
            media = [media]
        ...
        # make input ids
        input_ids = self.jamba_tokenizer_image_token(prompt, self.tokenizer, IMAGE_TOKEN_INDEX,
                                                     return_tensors='pt').unsqueeze(0).to(self.device)
        if self.images != [None]:
            lenth = len(images)
            image_tensors = self.jamba_process_images(self.images, self.processor, self.model.config).to(self.device,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--patchside_length', default=336, type=int,
                        help="the length of the subImage for image patching")
    parser.add_argument("--patchStrategy", type=str, default='norm',
                        help='Strategy to apply for patching. Options include "norm", "bestFit". "Norm" means resize the Image, "bestFit" means patch the image into subImages with patchside_length*patchside_length')
    ...
    while True:
        ...
        answer = bot.chat(images=images, text=text, isVideo=args.isVideo, t=args.t, frameNum=args.frameNum,
                          patchside_length=args.patchside_length, patchStrategy=args.patchStrategy)
