...
class LlavaJambaModel(LlavaMetaModel, paddlenlp.transformers.jamba.modeling
    .JambaModel):
...
class LlavaJambaForCausalLM(paddlenlp.transformers.jamba.modeling.
    JambaForCausalLM, LlavaMetaForCausalLM):
    config_class = LlavaJambaConfig
    ...
    def __init__(self, config):
        ...
        self.lm_head = paddlenlp.transformers.jamba.modeling.JambaLMHead(config
            )
    ...
    def forward(self, input_ids: paddle.Tensor=None, attention_mask:
        ...
        if inputs_embeds is None:
            (input_ids, position_ids, attention_mask, past_key_values,
                ...
                prepare_inputs_labels_for_multimodal(input_ids,
                position_ids, attention_mask, past_key_values, labels,
                images, image_sizes))
    ...
    def generate(self, inputs: Optional[paddle.Tensor]=None, images:
        ...
        if images is not None:
            inputs, position_ids, attention_mask, _, inputs_embeds, _ = (self
                .prepare_inputs_labels_for_multimodal(inputs, position_ids,
                attention_mask, None, None, images, image_sizes=image_sizes))
        else:
